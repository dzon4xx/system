...
from backend.components.dependancy import Dependancy, DependancyConfigError
from backend.components.element import Element, InputElement, OutputElement, Blind
from backend.components.module import Module, OutputBoard, LedLightBoard, AmbientBoard, InputBoard, AddElementError
from backend.components.regulation import Regulation, RegulationConfigError
from backend.misc.color_logs import color_logs
from backend.misc.sys_types import Mt, Et, Rt, Regt
...
class SystemCreator():
    ...
    def add_module(self, type=None, name=''):
        ...
        if type == Mt.ambient:
            module = AmbientBoard(self.module_id, type, name)
        elif type == Mt.input:
            module = InputBoard(self.module_id, type, name)
        elif type == Mt.output:
            module = OutputBoard(self.module_id, type, name)
        elif type == Mt.led_light:
            module = LedLightBoard(self.module_id, type, name)
    ...
    def add_element(self, type=None, name='', room_id=None, module_id=None, port=None,):
        ...
        try:
            if type == Et.dht:
                el1= InputElement(self.element_id, Et.dht_hum, 'Humidity', module_id, port)
                self.element_id += 1
                el2 = InputElement(self.element_id, Et.dht_temp, 'Temperature', module_id, port)
                self.element_id += 1
            ...
            elif type in InputElement.types:
                el = InputElement(self.element_id, type, name, module_id, port)
                self.element_id += 1
                Room.items[room_id].add_element(el)
                Module.items[module_id].add_element(port, el)

            elif type in OutputElement.types:
                el = OutputElement(self.element_id, type, name, module_id, port)
                self.element_id += 1
                Room.items[room_id].add_element(el)
                Module.items[module_id].add_element(port, el)
