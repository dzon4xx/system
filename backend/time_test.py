...
def test_delayus(delayus):
  ...
  tests   = [   0.1,  0.5,    1,  10,  50, 100, 500, 1000,  100000, 1000000]
  ...
  for i in range(len(n_tests)):
    ...
    print ("%10.1f usec delay: time = %0.3f usec, error = %0.3f, min = %0.3f, max = %0.3f" %\
           (tests[i], avg, error, min, max))
...
def test_delayms(delayms):  
  ...
  for i in range(len(n_tests)):
    ...
    if (t): 
      ...
      # it wouldn't make sense to include it in our average error.
    print ("{} msec delay: time = {} msec, error = {}".format(tests[i], avg, error))

  print ("\n  avg error = +- {} msec\n".format(total_error/len(n_tests)))

#------------------------------------


#print ("\n Microsecond delay using time.sleep():{}".format(test_delayus(sleepMicroseconds)))
#print (20*'-')
print ("\n Microsecond delay using ctypes and usleep():{}".format(test_delayus(delayMicroseconds)))
print (20*'-')
#print ("\n Microsecond delay using ctypes and nanosleep():{}".format(test_delayus(nanosleepMicroseconds)))
#print (20*'-')
#print ("\n Millisecond delay using time.sleep():{}".format(test_delayms(sleepDelay)))
#print (20*'-')
#print ("\n Millisecond delay using ctypes and usleep():{}".format(test_delayms(delay)))
#print (20*'-')
#print ("\n Millisecond delay using ctypes and usleep() with simple error compensation:".format(test_delayms(betterDelay)))
#print (20*'-')
